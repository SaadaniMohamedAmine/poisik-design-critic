...
## Phase 11 — Manual QA Bug Fixes (Complete — 4/4)

Found and fixed while manually testing every feature shipped in Phase 10, end-to-end, screenshot by screenshot.

- **Project Detail empty-state card collapsed to a narrow column**: root cause was `max-w-md`, a bare Tailwind default-scale class this project's custom theme doesn't define (the theme's own `spacing` scale reuses the same key names — `sm`/`md`/`lg`/`xl` — for padding/gap tokens, silently shadowing Tailwind's built-in `max-w-{xs,sm,md,lg,xl}` utilities; the numbered `2xl`–`7xl` tier is unaffected and used safely elsewhere in the app). Fixed to `max-w-[448px]`, matching the Dashboard's identical empty-state pattern. Commit `99cbce8`
- **Oversized CTA buttons on that same empty state**: once the card was fixed to its proper width, the "Run first analysis"/"View sample report" buttons read as too large. Shrunk padding/font-size/border-radius/icon-size, added `whitespace-nowrap`. Commit `dcdc223`
- **Stripe checkout redirected from localhost to the production Vercel URL after payment**: `NEXT_PUBLIC_SITE_URL` in `.env.local` (gitignored, not a code bug) was set to the deployed Vercel URL instead of `http://localhost:3000`; `lib/stripe.ts` uses this env var to build `success_url`/`cancel_url`/`return_url`. Fixed the env var locally; confirmed with the user that local and prod intentionally share one Neon database, so the resulting real PRO upgrade on the test account was expected, not a data-integrity issue. Not committed (env-only change)
- **Same `max-w-sm` bug recurred in the newly-built `UpgradeModal`**: identical root cause/symptom as the first bug (its `DialogDescription` used `max-w-sm`). Fixed to `max-w-[320px]`. Commit `f8906a4`. Swept the rest of the codebase for the same bug class afterward — every other `max-w-*` usage found (`2xl` through `7xl`, across headers/footers/marketing pages/ReportView) uses the numbered tier, which is unaffected, so no further instances exist
- **Established convention going forward**: never use bare `max-w-xs/sm/md/lg/xl` in this project — always an explicit arbitrary value (`max-w-[NNNpx]`), per the pattern already used everywhere else in the codebase before this bug was introduced twice
...
- **Notification system**: `Notification` Prisma model (userId, type, title, message, link, read), full API (`GET /api/notifications`, `PATCH .../[id]/read`, `PATCH .../read-all`), `NotificationBell` wired into both the marketing header and the authenticated topbar (hidd n when logged out)
